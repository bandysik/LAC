package lac.core.feature.core.utils
...
interface Pikkel {
    ...
    fun restoreInstanceState(savedInstanceState: Bundle?) {
        ...
        pikkelBundle.keySet()
            .filter { !it.startsWith("pikkel:") }
            .forEach { pikkelBundle.remove(it) }
    }
    ...
    fun <T> state(initial: T): ReadWriteProperty<Pikkel, T> =
            State(initial)
    ...
    private class State<T>(private val initial: T) : ReadWriteProperty<Pikkel, T> {
        ...
        override fun getValue(thisRef: Pikkel, property: KProperty<*>): T {
            ...
            if (!thisRef.pikkelBundle.containsKey(key)) {
                ...
                @Suppress("UNCHECKED_CAST") return thisRef.pikkelBundle.get(key) as T
            }
        }
